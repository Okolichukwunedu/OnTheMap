...
class ConfirmLocationViewController: UIViewController, UITextFieldDelegate, MKMapViewDelegate {
    ...
    @IBAction func Submit( sender: Any) {
        OTMUser.getUserInformation (completion: {(OTMUserResult, error) in
            ...
            let studentLocationRequest = PostStudentLocation(uniqueKey: OTMUserResult.accountKey, firstName: OTMUserResult.firstName, lastName: OTMUserResult.lastName, mapString: self.locationString, mediaURL: self.addLink.text!, latitude: self.location.latitude, longitude: self.location.longitude)
    ...
    @IBAction func cancelButton(_ sender: Any) {
        performSegue(withIdentifier: "backToMap", sender: sender)
    }
