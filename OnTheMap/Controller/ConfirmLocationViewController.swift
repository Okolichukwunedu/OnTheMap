...
class ConfirmLocationViewController: UIViewController, UITextFieldDelegate, MKMapViewDelegate {
    ...
    var locData: PostStudentLocation?
    ...
    func getMapInformation() {
        ...
        mapLocation.title = locData?.firstName
    ...
    @IBAction func Submit( sender: Any) {
        OTMUser.getUserInformation (completion: {(OTMUserResult, error) in
            ...
            let studentLocationRequest = PostStudentLocation(uniqueKey: OTMUserResult.accountKey, firstName: OTMUserResult.firstName, lastName: OTMUserResult.lastName, mapString: self.locData?.mapString ?? "", mediaURL: self.addLink.text!, latitude: self.location.latitude, longitude: self.location.longitude)
