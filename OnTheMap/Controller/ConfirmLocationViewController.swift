...
class ConfirmLocationViewController: UIViewController, UITextFieldDelegate, MKMapViewDelegate {
    ...
    var locationText: PostStudentLocation?
    ...
    func getMapInformation() {
        ...
        mapLocation.title = locationText?.firstName
    ...
    @IBAction func Submit( sender: Any) {
        OTMUser.getUserInformation (completion: {(OTMUserResult, error) in
            ...
            let studentLocationRequest = PostStudentLocation(uniqueKey: OTMUserResult.accountKey, firstName: OTMUserResult.firstName, lastName: OTMUserResult.lastName, mapString: self.locationText?.mapString ?? "", mediaURL: self.addLink.text!, latitude: self.location.latitude, longitude: self.location.longitude)
